...
| ~~Dropped item behaviour, in any form~~ | **rejected outright** | `EntityItem.onUpdate` measured 7.50 ms/tick, 20% of entity time, with 2.96 ms of that in `handleWaterMovement`. It is not a target. `EntityItem` already *overrides* `Entity.handleWaterMovement` with a leaner version — Mojang optimised it. The cost scales with item count, not with the method, and the item count came from deliberately killing hundreds of bosses. Every way to attack it either changes physics for every item from every mod, caches a value that goes stale when water changes, or deletes loot the player wanted. None is worth it. |
